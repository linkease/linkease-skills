...
[ "$canonical_count" -eq 19 ] || fail "expected 19 canonical iStoreOS skills, got $canonical_count"
...
[ "$remote_count" -eq 2 ] || fail "expected 2 remote transport skills, got $remote_count"
...
[ "$direct_count" -eq 21 ] || fail "direct preset install expected 21 skills, got $direct_count"
...
[ "$root_count" -eq 21 ] || fail "root installer expected 21 skills, got $root_count"

for installed in "$tmp/direct"/istoreos-*; do
  name="$(basename "$installed")"
  [ "$name" = "istoreos-ssh-ops" ] && continue

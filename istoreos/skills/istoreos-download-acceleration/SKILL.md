...
- For Homebrew installs, prefer `eval "$(iStoreEnhance brew-env --mode free)"` first. Free mode accelerates only Homebrew API metadata through `dl-homebrew-api`; Plus mode may add `HOMEBREW_BOTTLE_DOMAIN=https://ghcr.linkease.net:5443/v2/homebrew/core`. Do not default bottle/source/cask traffic to DomainFold/admin_proxy.
